...
export const RemarksHistoryModal: React.FC<RemarksHistoryModalProps> = ({
  ...
  return (
    ...
    >
      <div 
        className="w-[95%] sm:w-full max-w-lg max-h-[90vh] flex flex-col bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 overflow-hidden animate-in zoom-in-95 duration-150"
      ...
      >
          ...
          {remarks.length === 0 ? (
            ...
            remarks.map((rem, idx) => (
              <div key={rem.id || idx} className={`${idx > 0 ? 'pt-3' : ''} space-y-1.5`}>
                ...
                <div className="text-xs sm:text-sm text-slate-800 dark:text-slate-200 bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-100 dark:border-slate-800/60 leading-relaxed whitespace-pre-wrap">
                  {rem.text}
                </div>
              </div>
            ))
        ...
        <form onSubmit={handleSubmit} className="p-3 sm:p-4 bg-slate-50 dark:bg-slate-900/90 border-t border-slate-100 dark:border-slate-800">
          <label htmlFor="modal-new-remark" className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">
            Add New Remark
          </label>
          <div className="flex gap-2">
            <textarea
              ...
              className="flex-1 p-2.5 text-xs sm:text-sm bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 rounded-xl border border-slate-300 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-brand-500 transition-all resize-none"
            />
            <button
              type="submit"
