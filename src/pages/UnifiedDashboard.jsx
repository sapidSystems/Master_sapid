...
export default function UnifiedDashboard() {
  ...
  // User role and permissions from storage
  ...
  // Systems Definition for all 4 Operational Pipelines
  const allSystemTiles = useMemo(
    () => [
      {
        id: "sample",
        name: "Sample System",
        ...
        stagePreview: "Received • Handover • Ready • Dispatched",
      },
      {
        id: "production",
  ...
  // Compute allowed pipelines according to user role and permissions
  const allowedPipelines = useMemo(() => {
    return getAllowedPipelines(pageAccess, userRole);
  }, [pageAccess, userRole]);

  const allowedTiles = useMemo(() => {
    return allSystemTiles.filter((sys) => allowedPipelines.includes(sys.id));
  }, [allSystemTiles, allowedPipelines]);

  // Selected system state: automatically preselects if only 1 allowed, or respects permitted URL query
  const [activeSystem, setActiveSystem] = useState(() => {
    if (systemQuery && allowedPipelines.includes(systemQuery) && ALL_SYSTEM_CONFIGS[systemQuery]) {
      return systemQuery;
    }
    return allowedPipelines[0] || "sample";
  });

  // Reconcile activeSystem when permissions or allowed pipelines change
  useEffect(() => {
    if (allowedPipelines.length === 0) return;

    if (allowedPipelines.length === 1) {
      if (activeSystem !== allowedPipelines[0]) {
        setActiveSystem(allowedPipelines[0]);
        setSearchParams({ system: allowedPipelines[0] }, { replace: true });
      }
      return;
    }

    if (systemQuery && allowedPipelines.includes(systemQuery)) {
      if (activeSystem !== systemQuery) {
        setActiveSystem(systemQuery);
      }
    } else if (!allowedPipelines.includes(activeSystem)) {
      const fallback = allowedPipelines[0] || "sample";
      setActiveSystem(fallback);
      setSearchParams({ system: fallback }, { replace: true });
    }
  }, [allowedPipelines, systemQuery, activeSystem, setSearchParams]);
  ...
  // Load items for the active system with query guard
  const loadActiveSystemItems = useCallback(
    async (sysKey) => {
      if (!sysKey) return;

      // API/Query Guard: block fetching if user lacks permission for this report pipeline
      if (!hasPipelineReportAccess(sysKey, pageAccess, userRole)) {
        console.warn(`[Security Guard] Blocked unauthorized query for report pipeline: ${sysKey}`);
  ...
  useEffect(() => {
    if (allowedPipelines.includes(activeSystem)) {
      loadActiveSystemItems(activeSystem);
    }
  }, [activeSystem, loadActiveSystemItems, allowedPipelines]);

  const handleSelectSystem = (sysKey) => {
    if (!allowedPipelines.includes(sysKey)) return;
  ...
  const hasAnyAccess = hasAnyReportAccess(pageAccess, userRole) && allowedTiles.length > 0;
  const activeConfig = ALL_SYSTEM_CONFIGS[activeSystem] || sampleConfig;

  // Sum only permitted systems for header counter
  const totalPermittedCount = useMemo(() => {
    return allowedTiles.reduce((acc, sys) => acc + (sys.count || 0), 0);
  }, [allowedTiles]);
      ...
      <div className="w-full min-h-screen bg-slate-50/50 p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 p-3.5 sm:p-4 shadow-soft-sm relative z-30">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3 sm:gap-4 flex-wrap">
              ...
              <div className="bg-slate-900 text-white px-4 py-2.5 rounded-xl shadow-soft-sm flex items-center gap-3.5">
                ...
                <button
                  ...
                  disabled={countsLoading || isBoardLoading || !hasAnyAccess}
            ...
            {/* Right Side: Dropdown Selector Component (Only shows permitted pipelines) */}
            {hasAnyAccess && (
              <div className="relative" ref={dropdownRef}>
                {(() => {
                  const currentSystem =
                    allowedTiles.find((s) => s.id === activeSystem) || allowedTiles[0];
                  ...
                  return (
                    <>
                      <button
                        type="button"
                        onClick={() => {
                          if (allowedTiles.length > 1) {
                            setIsSystemDropdownOpen((prev) => !prev);
                          }
                        }}
                        disabled={allowedTiles.length <= 1}
                        className={`flex items-center justify-between gap-3 min-w-[280px] sm:min-w-[340px] bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-left transition-all shadow-2xs ${
                          allowedTiles.length > 1
                            ? "hover:bg-slate-100/80 hover:border-slate-300 cursor-pointer"
                            : "cursor-default"
                        } focus:outline-hidden focus:ring-2 focus:ring-brand-500/20`}
                      >
                        <div className="flex items-center gap-2.5 min-w-0">
                          <div
                            className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${currentSystem.iconBg}`}
                          >
                      ...
                      {isSystemDropdownOpen && allowedTiles.length > 1 && (
                        <div className="absolute right-0 mt-2 w-full sm:w-[360px] bg-white rounded-2xl border border-slate-200 shadow-xl p-1.5 space-y-1 z-50 animate-in fade-in-0 zoom-in-95 duration-150">
                          <div className="px-3 py-2 text-[11px] font-bold text-slate-400 uppercase tracking-wider border-b border-slate-100 mb-1">
                            Permitted Pipelines ({allowedTiles.length})
                          </div>
                          {allowedTiles.map((sys) => {
                            ...
                            return (
                              ...
                              >
                                <div className="flex items-center gap-3 min-w-0">
                                  <div
                                    className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${sys.iconBg}`}
                                  >
        ...
        {/* Kanban Board View or Empty Access State */}
        {!hasAnyAccess ? (
          <div className="flex flex-col items-center justify-center min-h-[420px] bg-white rounded-3xl border border-slate-200/80 p-8 text-center space-y-4 shadow-soft-sm">
            ...
            <div className="max-w-md">
              ...
              <p className="text-xs text-slate-500 mt-1 leading-relaxed">
                You do not currently have permission to view operational pipeline reports.
                Please contact an administrator to request report permissions for your profile.
              </p>
            </div>
            <button
              onClick={() => navigate("/dashboard/admin")}
              className="px-5 py-2.5 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-semibold shadow-soft-sm transition-all"
            >
              Go to Checklist Dashboard
            </button>
          </div>
        ) : (
