...
export default function ProductionPlanningNew() {
  ...
  const handleOpenScheduleModal = (plan) => {
    ...
    // All Stage-by-Stage Milestone Targets load blank — no auto-filled Planned Dates
  ...
  const handleSaveModalSchedule = async (e) => {
    ...
    // Validate that Planned Shipment is <= Work Order Shipment Date
    const shipmentStage = stageDates.find(stg => stg.name === 'Planned Shipment');
    if (shipmentStage?.plannedDate && selectedPlan.woDespatchDate && shipmentStage.plannedDate > selectedPlan.woDespatchDate) {
      showToast(`Planned Shipment date cannot be later than Work Order Shipment Date (${formatDate(selectedPlan.woDespatchDate)})`, 'error');
      return;
    }

    try {
      setIsSubmitting(true);
      const finalStages = stageDates;

      const planDateToSave = selectedPlan.planDate || finalStages.find(s => s.plannedDate)?.plannedDate || getTodayDate();
      await submitForApproval(selectedPlan.id, planDateToSave, finalStages);
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
            <div className="bg-white rounded-2xl max-w-2xl w-full p-6 shadow-2xl space-y-4">
              ...
              <form onSubmit={handleSaveModalSchedule} className="space-y-4">
                {/* Read-only Work Order Parameters */}
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <input
                              type="date"
                              max={isShipmentStage && selectedPlan.woDespatchDate ? selectedPlan.woDespatchDate : undefined}
                              value={stg.plannedDate}
                              onChange={(e) => {
                                const val = e.target.value;
                                if (isShipmentStage && selectedPlan.woDespatchDate && val && val > selectedPlan.woDespatchDate) {
                                  showToast(`Planned Shipment date cannot be later than ${formatDate(selectedPlan.woDespatchDate)}`, 'error');
                                  return;
                                }
                                setStageDates(prev =>
                              ...
                              className="px-2.5 py-1 rounded-lg text-xs font-medium border bg-white border-slate-300 text-slate-800 focus:border-black outline-hidden"
                            />
                          </div>
                        </div>
                      );
