...
export default function ProductionApproval() {
  ...
  const handleRejectFromModal = async () => {
    ...
    if (!decisionNote.trim()) {
      showToast('Please enter a rejection reason in the note field before rejecting', 'error');
      return;
    }
    ...
    try {
      ...
      showToast(`Work order ${selectedPlan.woNo} returned for revision.`, 'info');
      await loadData();
  ...
  const handleConfirmApprove = async (e) => {
    ...
    if (!selectedPlan) return;
    const finalPlanDate = planDateInput || selectedPlan.planDate || getTodayDate();
          ...
          <div className="max-w-7xl mx-auto flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-2 text-xs font-semibold text-slate-500 uppercase tracking-wider">
                ...
                <span>•</span>
                <span className="text-brand-600">Module 4 of 5</span>
              </div>
              ...
              <div className="overflow-y-auto pr-1 space-y-4 flex-1">
                ...
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-2">
                  <label className="block text-xs font-bold text-slate-700">
                    Decision Note / Reason for Rejection / Remarks
                  </label>
                  <input
                    type="text"
                    placeholder="e.g. Approved for procurement kickoff, or reason for rejection..."
                    value={decisionNote}
              ...
              <div className="flex items-center justify-between pt-3 border-t border-slate-100 shrink-0">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-4 py-2 text-sm font-bold text-rose-700 bg-rose-50 hover:bg-rose-100 active:bg-rose-200 border border-rose-200 rounded-xl transition-all cursor-pointer inline-flex items-center gap-1.5"
                  >
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
            <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-4">
              ...
              <p className="text-xs text-slate-600">
                Rejecting will return this plan to <strong>Work Order Creation</strong> in rejected status so the team can revise dates or quantities.
              </p>
