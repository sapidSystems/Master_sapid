...
const mapEnquiryDbToLead = (dbRow) => {
...
export default function SampleDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      setIsLoading(true);
      const { data, error } = await supabase.from('sample_system_sample_management').select('*');

      if (error) throw error;

      setEnquiries((data || []).map(mapEnquiryDbToLead));
    } catch (err) {
      console.error('Error fetching dashboard data:', err);
      toast.error('Failed to load live data for dashboard');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const leads = useMemo(() => { 
    return dashboardType === 'enquiry' ? enquiries : bulkOrders;
  }, [dashboardType, enquiries, bulkOrders]);
  ...
  const leadsByType = useMemo(() => {
    ...
    filteredLeads.forEach(l => {
      const type = dashboardType === 'enquiry' ? (l.type || 'Unknown') : (l.buyer || 'Unknown');
      if (!typeData[type]) {
        typeData[type] = 0;
      }
      typeData[type] += 1;
